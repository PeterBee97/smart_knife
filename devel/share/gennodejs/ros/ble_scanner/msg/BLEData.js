...
let BLEBeacon = require('./BLEBeacon.js');
...
class BLEData {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.data.forEach((val) => {
      bufferOffset = BLEBeacon.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.data[i] = BLEBeacon.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.data.forEach((val) => {
      length += BLEBeacon.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new BLEData(null);
    ...
    if (msg.data !== undefined) {
      ...
      for (let i = 0; i < resolved.data.length; ++i) {
        resolved.data[i] = BLEBeacon.Resolve(msg.data[i]);
      }
    }
    else {
      resolved.data = []
